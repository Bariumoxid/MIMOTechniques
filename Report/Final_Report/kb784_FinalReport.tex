...
%%% Title
\title{\LARGE 
	Technical Abstract \\ \vspace{0.6cm}
	\textbf{CSI-Feedback-Driven Link Adaptation in 5G NR Downlink MIMO: A Link-Level Simulation Study}}
\author{
	Kaiyuan Bao (kb784)\\
...
\begin{highlightblock}
	\begin{highlightblock}
		...
		\begin{itemize}
			...
			\item \textbf{Section~3: Model Validation and Baseline CSI-Feedback Downlink Behaviour} validates the simulation model by comparing the fixed-MCS BLER curve with reported reference results from different firms. It then presents the baseline CDL-C \(4 \times 4\) CSI-feedback-driven throughput behaviour and analyses the CSI-driven link adaptation statistics.
			
			\item \textbf{Section~4: Robustness Across MIMO and Channel Configurations} investigates whether the observed behaviour remains consistent under different spatial and channel conditions. This includes antenna configuration sweeps, CDL channel profile sweeps, and one-layer control experiments.
			
			\item \textbf{Section~5: Impact of Mobility and CSI Reporting Periodicity} forms the central CSI-focused part of the report. It studies how mobility and CSI reporting periodicity affect CSI-feedback-driven link adaptation, and evaluates whether more frequent CSI updates improve transmitter adaptation and downlink throughput. The section also discusses the trade-off between CSI freshness, reporting overhead, and throughput gain.
...
\label{sec:2}
\begin{highlightblock}
	This section introduces the technical background and simulation methodology used in this project. The aim is to cover basic physical-layer related concepts and modelling choices required to study CSI-feedback-driven link adaptation in 5G NR downlink MIMO. 
	
	In the first three subsections of this section, an overview of the relevant 5G NR concepts, including OFDM resource allocation, PDSCH transmission, DM-RS, CSI-RS, and the performance metrics used in this study, are introduced. It then describes the MIMO and CSI feedback framework, such as how the UE estimates the downlink channel and reports CQI, PMI, and RI to support modulation, coding, precoding, and rank adaptation at the transmitter. These mechanisms form the basis of the closed-loop link adaptation behaviour investigated in the later chapters.
	
	The second part of the chapter describes the common 3GPP-aligned configuration assumptions used during the simulation. The MATLAB 5G Toolbox simulation framework used and the simulations to-be-conducted are also discussed.
\end{highlightblock}
...
\begin{highlightblock}
	Throughput is the main performance metric used in this study. It measures the amount of successfully delivered information per unit time, commonly expressed in terms of megabits per second (Mbps), and therefore reflects both reliability and spectral efficiency. A high modulation order or high code rate may increase the potential data rate, but it only improves throughput if the transport blocks can still be decoded successfully. Similarly, transmitting more MIMO layers can increase the peak throughput, but may also increase the risk of decoding failure if the channel cannot support the selected rank. In real-life applications, throughput is also a major performance criteria evaluated by the network operator, as the value directly impacts the performance of the network and is directly associated with the user experience.
\end{highlightblock}



\begin{highlightblock}
	In practical 5G NR downlink transmission, hybrid automatic repeat request (HARQ or hybrid-ARQ) is used to improve reliability. After DL-SCH decoding, the UE sends an acknowledgement or negative acknowledgement (ACK/NACK) to indicate whether the transport block has been decoded successfully. If decoding fails, the gNB may retransmit the transport block using a different redundancy version, allowing the UE to combine multiple transmissions using so called soft-combining and improve the probability of successful decoding. In this project, HARQ is included in the fixed-MCS validation experiment where comparison with reference BLER results is required, while it is disabled in the main CSI-feedback-driven throughput studies in order to isolate the effect of CSI-based link adaptation \cite[Ch.~9]{EricssonBook} \cite{MathWorks2026}.
\end{highlightblock}

	\begin{highlightblock}
	In summary, the downlink physical-layer components introduced in this section provide the basis for the link-level simulations in this project. PDSCH carries the user data, DM-RS supports coherent demodulation, CSI-RS supports channel measurement for feedback generation, and BLER and throughput are used to evaluate decoding reliability and delivered data rate. 
\end{highlightblock}
...
\end{highlightblock}
	...
	\begin{highlightblock}
		...
		Only a subset of the parameters under investigation is varied in each experiment, while the remaining parameters are kept fixed at their baseline values to ensure a controlled comparison. Table \ref{tab:sim_studies} shows a brief summary of the simulation study groups that are going to be simulated.
	\end{highlightblock}
