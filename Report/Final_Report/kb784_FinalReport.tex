...
\begin{highlightblock}
	Reference signals are known signals transmitted over selected resource elements, allowing the receiver to estimate or measure the channel for purposes such as data demodulation, channel state information (CSI) feedback, and phase tracking. While there are many types of reference signals, two of which are particularly important and relevant in this study: demodulation reference signals (DM-RS) and	channel-state-information reference signals (CSI-RS). 
...
\begin{highlightblock}
	In practical 5G NR downlink transmission, hybrid automatic repeat request (HARQ) is
	also used to improve reliability. After DL-SCH decoding, the UE sends an
	acknowledgement or negative acknowledgement (ACK/NACK) to indicate whether the
	transport block has been decoded successfully. If decoding fails, the gNB may
	retransmit the transport block using a different redundancy version, allowing the UE to
	combine multiple transmissions and improve the probability of successful decoding.
	HARQ therefore affects the relationship between BLER and the final delivered
	throughput. In this project, HARQ is included in the fixed-MCS validation experiment
	where comparison with reference BLER results is required, while HARQ is disabled in the main CSI-feedback-driven throughput studies in
	order to isolate the effect of CSI-based link adaptation.
\end{highlightblock}


\begin{highlightblock}
	In the simulated downlink chain, the transmitter selects the PDSCH transmission
	parameters based on the available CSI feedback. The encoded and modulated data are
	mapped onto the resource grid together with the required reference signals, precoded across the transmit antennas, and transmitted through a time-varying MIMO channel.
	
	At the receiver, synchronisation, OFDM demodulation, channel estimation, equalisation, PDSCH decoding, and DL-SCH decoding are performed. The resulting BLER and throughput are then used to evaluate the behaviour of the downlink link adaptation mechanism under different channel, antenna, mobility, and CSI reporting
	configurations. <><><> Should I put this part next Chpt. ???
\end{highlightblock}
	
	
In summary, the downlink physical-layer components introduced in this section provide
the basis for the link-level simulations in this project. PDSCH carries the user data,
DM-RS supports coherent demodulation, CSI-RS supports channel measurement for
feedback generation, and BLER and throughput are used to evaluate decoding
reliability and delivered data rate. The way in which CSI feedback controls the PDSCH
transmission parameters is discussed in the next section.

2.2: The CSI-based downlink transmission and feedback framework used in this project is
summarised in Fig.~\ref{fig:csi-feedback-framework}. At the transmitter, the DL-SCH
transport block is processed and mapped to the PDSCH. The PDSCH symbols are then
precoded across the transmit antennas and passed through CP-OFDM modulation before
being transmitted through the MIMO channel. At the receiver, timing synchronisation,
CP-OFDM demodulation, DM-RS-based channel estimation, PDSCH decoding, and
DL-SCH decoding are performed.

In parallel with data demodulation, the UE measures CSI-RS resources to estimate the
downlink channel for feedback purposes. The resulting CSI report is encoded and fed
back to the gNB, where it is decoded and used to select the number of layers, the MCS,
and the precoding matrix for subsequent PDSCH transmissions. This closed-loop
operation is the basis of the CSI-feedback-driven link adaptation studied in this
project. Fig.~\ref{fig:csi-feedback-framework} also shows the HARQ feedback path. After
DL-SCH decoding, the receiver generates an ACK/NACK decision. A failed transport
block can be retransmitted and combined with previous transmissions, improving
decoding reliability. HARQ is therefore a reliability mechanism that operates alongside
CSI feedback. However, it serves a different purpose: CSI feedback adapts future
transmission parameters, while HARQ responds to decoding success or failure of
previous transmissions.
	
	
\subsection{MIMO Transmission and CSI Feedback}
	\begin{highlightblock}
		MIMO transmission is a key technique used in 5G NR
		to improve downlink performance. By using multiple transmit and receive antennas,
		the system can exploit the spatial dimension of the wireless channel. Depending on the
		channel condition and antenna configuration, MIMO can provide array gain,
		beamforming gain, diversity gain, or spatial multiplexing gain. This project focuses
		mainly on spatial multiplexing and CSI-driven link adaptation, where multiple data
		layers may be transmitted over the same time-frequency resources in order to increase
		throughput.
		
		In spatial multiplexing, the transmitted data stream is divided into one or more
		spatial layers. The number of layers is often referred to as the transmission rank. If
		the channel provides sufficiently independent spatial paths, increasing the rank allows
		more information to be transmitted in parallel, which can significantly increase the
		achievable throughput. However, using a higher rank is not always beneficial. If the
		channel is highly correlated, the received signal-to-noise ratio is low, or the receiver
		cannot reliably separate the layers, then high-rank transmission may lead to a higher
		block error rate and lower throughput. Rank selection is therefore an important part
		of MIMO link adaptation.
	\end{highlightblock}
	
	\begin{highlightblock}
		Precoding is used to map the transmission layers onto the physical transmit antennas.
		The purpose of precoding is to shape the transmitted signal according to the spatial
		characteristics of the channel. In a feedback-based system, the transmitter does not
		usually have perfect and instantaneous knowledge of the downlink channel. Instead, the
		UE estimates the channel from reference signals and reports quantised CSI back to the
		gNB. The gNB then uses this feedback to select suitable transmission parameters,
		including the modulation and coding scheme, the number of layers, and the precoding
		matrix.
	\end{highlightblock}
	
	\begin{highlightblock}
		The CSI feedback considered in this project consists mainly of three components: the
		channel quality indicator (CQI), the rank indicator (RI), and the precoding matrix
		indicator (PMI). CQI represents the UE's estimate of the downlink channel quality. It
		is related to the highest modulation and coding scheme that can be supported while
		satisfying a target block error rate requirement. A higher CQI generally corresponds
		to a more aggressive transmission format, such as a higher modulation order or higher
		target code rate, while a lower CQI leads to a more robust but lower-rate transmission.
	\end{highlightblock}
	
	\begin{highlightblock}
		The rank indicator (RI) reports the number of spatial layers recommended by the UE.
		This quantity is directly connected to spatial multiplexing. A low RI indicates that the
		channel is better suited to fewer layers, while a higher RI indicates that the channel
		can potentially support multiple parallel layers. Since rank affects both the peak data
		rate and the reliability of the transmission, RI adaptation is one of the central
		mechanisms studied in this project. The fixed-rank control experiments in later
		sections are designed to isolate the effect of this rank adaptation mechanism.
	\end{highlightblock}
	
	\begin{highlightblock}
		The precoding matrix indicator (PMI) specifies the preferred precoder from a
		predefined codebook. The selected precoder determines how the transmission layers are
		mapped onto the transmit antenna ports. A suitable PMI allows the transmitter to
		better match the spatial structure of the channel, improving the received signal quality
		and reducing inter-layer interference. In practical systems, the PMI is quantised and
		therefore cannot describe the channel perfectly, but it still provides useful spatial
		information for downlink MIMO transmission.
	\end{highlightblock}
	
	\begin{highlightblock}
		The overall CSI feedback loop can be summarised as follows. The gNB transmits
		CSI-RS resources, which are measured by the UE. Based on these measurements, the UE
		estimates the downlink channel and generates CSI reports containing CQI, RI, and PMI.
		These reports are fed back to the gNB. The gNB then uses the reported information to
		select the PDSCH transmission parameters, including the modulation scheme, target
		code rate, number of layers, and precoder. The selected configuration is then used for
		subsequent downlink data transmission. This creates a closed-loop link adaptation
		process in which the transmitter continuously adjusts its transmission strategy based
		on the receiver's channel measurements.
	\end{highlightblock}
	
	\begin{highlightblock}
		The accuracy and freshness of CSI feedback are therefore critical to downlink
		performance. If the reported CSI accurately represents the channel at the time of data
		transmission, the gNB can choose a suitable MCS, rank, and precoder. If the CSI is
		outdated or too coarsely quantised, the selected transmission parameters may no
		longer match the actual channel. For example, the transmitter may choose a modulation
		and coding scheme that is too aggressive, increasing the BLER, or it may choose a
		conservative configuration that fails to exploit the available channel capacity.
		
		CSI reporting periodicity is one of the main parameters controlling this trade-off. A
		shorter reporting period provides more frequent CSI updates and can improve link
		adaptation in time-varying channels. However, frequent CSI reporting also increases
		feedback overhead and may consume additional uplink resources and UE power. A
		longer reporting period reduces this overhead, but increases the risk that the
		transmitter uses outdated CSI, especially when the channel varies rapidly due to UE
		mobility. The choice of CSI reporting period is therefore not simply a matter of making
		feedback as frequent as possible, but rather a trade-off between CSI freshness,
		feedback overhead, and the resulting downlink throughput.
		
		In this project, CSI feedback is studied through its impact on downlink link
		adaptation and throughput. The simulation framework allows the CQI, RI, and PMI
		feedback to influence the selected MCS, number of layers, and precoding matrix. The
		resulting behaviour is then evaluated under different antenna configurations, channel
		models, Doppler shifts, and CSI reporting periods. This makes it possible to examine
		how the effectiveness of CSI-driven link adaptation depends on both the spatial
		properties of the MIMO channel and the time variation of the wireless link.
	\end{highlightblock}
